...
export interface HistoryItem {
  ...
  tabType: "track" | "playlist"; // Which tab this item belongs to
  name: string;
  artist: string;
  image: string;
  timestamp: number;
...
interface FetchHistoryProps {
  ...
  tabFilter?: "track" | "playlist"; // Filter history by tab type
}

export function FetchHistory({ history, onSelect, onRemove, tabFilter }: FetchHistoryProps) {
  // Filter history based on tab type
  const filteredHistory = tabFilter 
    ? history.filter(item => item.tabType === tabFilter)
    : history;
