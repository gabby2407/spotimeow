...
const HISTORY_KEY = "spotimeow_fetch_history";
const MAX_HISTORY = 5;
...
function App() {
  ...
  const checkForUpdates = async () => {
    try {
      ...
      // Just check for updates without storing the state
      // since the variables are not used in the UI
  ...
  const loadHistory = () => {
    try {
      ...
      if (saved) {
        ...
        // Handle legacy history items without tabType
        const updatedHistory = parsedHistory.map((item) => {
          if (!item.tabType) {
            // Assign tabType based on item type
            item.tabType = (item.type === "track" || item.type === "album") ? "track" : "playlist";
          }
          return item;
        });
  ...
  const handleNewSearch = () => {
    // Clear current results and URL to start fresh
    setSpotifyUrl("");
    metadata.clearMetadata();
  };

  const handleFetchMetadata = async (searchTab?: "track-url" | "playlist-url" | "title-search") => {
    let expectedType: "track" | "playlist" | undefined = undefined;
    
    if (searchTab === "track-url") {
      expectedType = "track";
    } else if (searchTab === "playlist-url") {
      expectedType = "playlist";
    }

    const updatedUrl = await metadata.handleFetchMetadata(spotifyUrl, expectedType);
    if (updatedUrl) {
      setSpotifyUrl(updatedUrl);
    }
  };
          ...
          <>
            ...
            <SearchBar
              ...
              onFetchUrl={async (url) => {
                setSpotifyUrl(url);
                // For search results, we don't enforce tab validation
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 ml-48 mt-10">
          <div className="max-w-4xl mx-auto space-y-1 p-4 md:p-8">
            {renderPage()}
          </div>
        </div>
